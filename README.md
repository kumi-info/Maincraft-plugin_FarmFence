...
- **落下ランタンの撤去（v1.10.0）**: `frame.clear-dropped-items: true`（既定）で、アリーナ拡張時に浮いたランタン等が落下して**ドロップアイテム化**し農場内に残るのを防ぎます。枠を作り直す/消すたびに、枠領域に落ちた枠素材のアイテムを撤去します。
...
| バージョン | 変更点 |
|---|---|
| v1.10.0 | アリーナ拡張時に浮いたランタン等が落下して**ドロップアイテム化**し農場内に残る不具合を修正。枠を作り直す/消すときに、枠領域（±1マス）に落ちた枠素材のアイテムを撤去する `frame.clear-dropped-items`（既定ON）を追加。 |
